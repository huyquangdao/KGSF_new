...
from dataset_copy_helpful import dataset, CRSdataset
...
#                 batch_entities.extend(movie.detach().cpu().numpy().tolist())
    ...
    def val(self, is_test=False):
        ...
        f = open("mim_context_test.txt", "w", encoding="utf-8")
        f.writelines([" ".join(sen) + "\n" for sen in context_sum])
        f.close()

        f = open("mim_output_test.txt", "w", encoding="utf-8")
