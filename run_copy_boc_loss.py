...
from dataset_copy_boc_loss import dataset, CRSdataset
from model_copy_boc_loss import CrossModel
...
class TrainLoop_fusion_gen:
    ...
    def train(self):
        ...
        for i in range(self.epoch * 3):
            ...
            for (
                ...
                joint_loss = gen_loss + 0.25 * mask_loss
    ...
    def val(self, is_test=False):
        ...
        f = open("boc_context_test.txt", "w", encoding="utf-8")
        f.writelines([" ".join(sen) + "\n" for sen in context_sum])
        f.close()

        f = open("boc_output_test.txt", "w", encoding="utf-8")
